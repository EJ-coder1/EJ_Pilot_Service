...
function calcPrice() {
  ...
  if (mmrServer === "global") {  // only apply surcharge for explicit global
...
function setupHeroModeToggle() {
  $$(".hero-mode-opt").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      renderHeroes(); // re-render to apply/remove lock
      // Lock/unlock the specific hero dropdown visually
      const specificDropWrap = document.getElementById("specificHeroDropWrap");
      const specificDrop = document.getElementById("specificHeroDrop");
      if (specificDrop) specificDrop.classList.toggle("hero-drop-locked", heroMode === "general");
      updateAll();
    });
  });
