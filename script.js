...
/*
  ...
  Top Rank     — optional target inside MMR; no separate surcharge
*/
const PRICING = {
  ...
  /* PH and Global MMR use fixed per-star rates. No percentage surcharge. */
  MMR_RATES: {
    "Grandmaster":       35,
    "Epic":              35,
    "Legend":            35,
...
/*
  ...
  Difficulty remains a visual hero tag only. It does not add a separate fee.
*/
const HEROES = [
...
/* Top Rank is an optional MMR target. It has no separate surcharge. */
...
function calcPrice() {
  ...
  const total = ratePerStar * starsNeeded;
...
function updatePriceBreakdown() {
  const { total, starsNeeded, valid, warning, ratePerStar, mode } = calcPrice();
  ...
  if (rib) {
    if (warning) {
      ...
      const choices = [];
      if (heroMode === "specific") choices.push("Specific Hero");
  ...
  if (show) {
    const base = $("#pbBase");
    if (base) base.textContent = `${formatPeso(ratePerStar)}/star`;

    ["pbHeroRow", "pbDiffRow"].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.hidden = heroMode !== "specific";
    });
    const hero = $("#pbHero");
    if (hero) hero.textContent = "Included";
    const diff = $("#pbDiff");
    if (diff) diff.textContent = "Included";
...
async function createOrderMessage() {
  ...
  const { starsNeeded, valid, total, ratePerStar } = calcPrice();
  const estimate = valid && total > 0 ? formatPeso(total) : "To confirm";
  const heroLabel = effectiveHero ? `${effectiveHero.name} (${effectiveHero.role})` : "General — any hero";
  const mmrTopLabel = mmrTopTarget ? ` (Top ${mmrTopTarget})` : "";
