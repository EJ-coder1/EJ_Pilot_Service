...
function populateRanks() {
  ...
  /* Wire division pickers — run after DOM is built */
  setupDivPicker($("#currentDivPicker"), "currentDiv", () => {
    updateStarsLabelsAndLimits();
    updateAll();
  });
  setupDivPicker($("#targetDivPicker"), "targetDiv", () => {
    updateStarsLabelsAndLimits();
    updateAll();
  });
}
...
/* ===================== DIVISION PICKERS ===================== */
/*
  For Grandmaster / Epic / Legend:
    Division V = bottom (worst), Division I = top (best)
    5 stars per division, 0–4 stars within a division
    Absolute star position = (5 - div) * 5 + starsInDiv
      e.g. Div V star 0 = absolute 0, Div I star 4 = absolute 24
  For Mythic+:
    No divisions. User inputs cumulative stars directly.
*/

function setupDivPicker(pickerEl, hiddenId, onChange) {
  if (!pickerEl) return;
  pickerEl.querySelectorAll(".div-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      pickerEl.querySelectorAll(".div-btn").forEach(b => {
      ...
      if (hidden) hidden.value = btn.dataset.div;
      onChange?.();
    });
  });
}

/* Returns absolute star index (0-based from rank bottom) from div + starsInDiv */
function divToAbsolute(divNum, starsInDiv) {
  // divNum: 5=bottom, 1=top; starsInDiv: 0–4
  return (5 - parseInt(divNum)) * 5 + parseInt(starsInDiv || 0);
}

/* Show/hide division pickers based on whether the rank uses divisions */
function updateStarsLabelsAndLimits() {
  ...
  const curDivRow  = $("#currentDivRow");
  const tgtDivRow  = $("#targetDivRow");
  const curInput   = $("#currentStars");
  const tgtInput   = $("#stars");
  const curLabel   = $("#currentStarsFieldLabel");
  const tgtLabel   = $("#targetStarsFieldLabel");
  const curHdLabel = $("#currentStarsLabel");
  const tgtHdLabel = $("#targetStarsLabel");

  if (!curTier || !tgtTier) return;

  // ── Current rank ──────────────────────────────────────────────────────────
  if (curTier.isPoints) {
    // Mythic+ : hide division row, show cumulative star input
    if (curDivRow) curDivRow.style.display = "none";
    if (curHdLabel) curHdLabel.textContent = "Current stars (cumulative)";
    if (curLabel) curLabel.textContent = `Stars ${curTier.minPoints}–${curTier.maxPoints < 9999 ? curTier.maxPoints : "999"}`;
    if (curInput) {
      curInput.min  = curTier.minPoints;
      curInput.max  = curTier.maxPoints < 9999 ? curTier.maxPoints : 999;
      curInput.placeholder = `${curTier.minPoints}–${curTier.maxPoints < 9999 ? curTier.maxPoints : "999"}`;
      if (parseInt(curInput.value) < curTier.minPoints) curInput.value = curTier.minPoints;
    }
  } else {
    // GM / Epic / Legend: show division picker, limit stars to 0–4
    if (curDivRow) curDivRow.style.display = "";
    if (curHdLabel) curHdLabel.textContent = "Current position";
    if (curLabel) curLabel.textContent = "Stars in this division (0–4)";
    if (curInput) { curInput.min = 0; curInput.max = 4; curInput.placeholder = "0–4"; }
  }

  // ── Target rank ───────────────────────────────────────────────────────────
  if (tgtTier.isPoints) {
    if (tgtDivRow) tgtDivRow.style.display = "none";
    if (tgtHdLabel) tgtHdLabel.textContent = "Target stars (cumulative)";
    if (tgtLabel) tgtLabel.textContent = `Stars ${tgtTier.minPoints}–${tgtTier.maxPoints < 9999 ? tgtTier.maxPoints : "999"}`;
    if (tgtInput) {
      tgtInput.min  = tgtTier.minPoints;
      tgtInput.max  = tgtTier.maxPoints < 9999 ? tgtTier.maxPoints : 999;
      tgtInput.placeholder = `${tgtTier.minPoints}–${tgtTier.maxPoints < 9999 ? tgtTier.maxPoints : "999"}`;
      if (parseInt(tgtInput.value) < tgtTier.minPoints) tgtInput.value = tgtTier.minPoints;
    }
  } else {
    if (tgtDivRow) tgtDivRow.style.display = "";
    if (tgtHdLabel) tgtHdLabel.textContent = "Target position";
    if (tgtLabel) tgtLabel.textContent = "Stars in this division (0–4)";
    if (tgtInput) { tgtInput.min = 0; tgtInput.max = 4; tgtInput.placeholder = "0–4"; }
  }
}

/* ── Get effective "absolute stars" for calculation ── */
/* For div ranks, we convert div+stars to absolute. For Mythic+, direct. */
function getEffectiveStars(rankName, rawInput, divId) {
  const tier = tierByName(rankName);
  if (!tier) return parseInt(rawInput) || 0;
  if (tier.isPoints) return parseInt(rawInput) || tier.minPoints;
  // div-based rank
  const div = parseInt($("#" + divId)?.value || "3");
  const starsInDiv = Math.min(4, Math.max(0, parseInt(rawInput) || 0));
...
function computeStarsNeeded() {
  ...
  /* For div-based ranks, convert division+stars → absolute star index.
     For Mythic+, use the cumulative star input directly. */
  const curStars = getEffectiveStars(curName, $("#currentStars")?.value, "currentDiv");
  const tgtStars = getEffectiveStars(tgtName, $("#stars")?.value, "targetDiv");
...
async function createOrderMessage() {
  ...
  const lines = [
    ...
    "",
    `Name: ${customerName}`,
    `Current Rank: ${curName}${!tierByName(curName)?.isPoints ? " Division " + ($("#currentDiv")?.value ? ["","I","II","III","IV","V"][parseInt($("#currentDiv").value)] : "") : ""}`,
    `Current Stars: ${!tierByName(curName)?.isPoints ? (getEffectiveStars(curName, $("#currentStars")?.value, "currentDiv") + " (Div " + (["","I","II","III","IV","V"][parseInt($("#currentDiv")?.value||3)] || "") + ", " + ($("#currentStars")?.value||0) + " stars)") : ($("#currentStars")?.value || 0) + " cumulative stars"}`,
    `Target Rank: ${tgtName}${!tierByName(tgtName)?.isPoints ? " Division " + ($("#targetDiv")?.value ? ["","I","II","III","IV","V"][parseInt($("#targetDiv").value)] : "") : ""}`,
    `Target Stars: ${!tierByName(tgtName)?.isPoints ? (getEffectiveStars(tgtName, $("#stars")?.value, "targetDiv") + " (Div " + (["","I","II","III","IV","V"][parseInt($("#targetDiv")?.value||3)] || "") + ", " + ($("#stars")?.value||1) + " stars)") : ($("#stars")?.value || 1) + " cumulative stars"}`,
