...
function _applyPosPicker(side, tier) {
  ...
  if (tier.isPoints) {
    /* Mythic+ uses ONE normal number input.
       IMPORTANT: do not rebuild that input on every keystroke.
       Rebuilding it was the bug that prevented deletion/editing. */
    ...
    const inputId = side === "cur" ? "currentStars" : "stars";
    let input = starRow.querySelector(".pos-num-input");

    /* If the correct input already exists for this tier, keep it intact.
       This preserves the caret, blank state, and whatever the user is typing. */
    if (input && input.dataset.rank === tier.name) {
      input.min = String(min);
      input.max = String(max);
      input.placeholder = `${min}–${tier.maxPoints < 9999 ? tier.maxPoints : "999"}`;
      return;
    }

    /* First time in this rank, or rank changed: preserve a valid previous value
       where possible, otherwise start at the tier minimum. */
    const previousRaw = input ? input.value : (starHid?.value ?? "");
    const previous = Number(previousRaw);
    const nextValue = Number.isFinite(previous) && previous >= min && previous <= max
      ? String(previous)
      : String(min);
    ...
    starRow.innerHTML = `
      <label class="pos-num-label">
        ...
        <input class="pos-num-input" id="${inputId}"
          data-rank="${tier.name.replace(/"/g, '&quot;')}"
          type="number" min="${min}" max="${max}" value="${nextValue}"
          inputmode="numeric" placeholder="${min}–${tier.maxPoints < 9999 ? tier.maxPoints : "999"}">
      </label>`;

    input = starRow.querySelector(".pos-num-input");
    if (input) {
      input.addEventListener("input", () => {
        /* Let the user freely edit/delete the field.
           Do not write back into the input during input events. */
        updateAll();
      });
      ...
      input.addEventListener("blur", () => {
        ...
        if (raw === "") {
          ...
          input.value = String(Math.max(min, Math.min(max, Math.trunc(value))));
        }
        updateAll();
      });
    }
  } else {
    /* Division ranks: show division pills + star taps 0–5. */
...
function setupOrderFlow() {
  ...
  ["currentRank", "targetRank", "preferredRole"].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
  ...
  /* currentStars / stars are dynamically-created for Mythic+.
     Their own listeners are installed by _applyPosPicker(), so they are
     deliberately NOT bound here. This prevents input rebuild loops. */
