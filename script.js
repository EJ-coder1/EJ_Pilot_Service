...
/* ===================== CONFIGURATION ===================== */
const SITE = {
  // Use the exact Facebook share/profile URL you provided.
...
function selectHero(name) {
  ...
  // Only auto-advance when the user entered the hero picker through Step 2.
  // Browsing heroes normally should NOT yank the user away from the grid.
...
function stepIsValid(step) {
  if (step === 1) {
    ...
    const starCount = parseInt(stars?.value, 10);
    if (!Number.isFinite(starCount) || starCount < 1) {
      stars?.focus();
      showToast("Enter at least 1 star or point.");
      return false;
    }
  }

  if (step === 2 && !selectedHero) {
...
function updateStepTabs() {
  $$("[data-step-tab]").forEach(tab => {
    ...
    const available = step <= orderStep && (step !== 3 || Boolean(selectedHero));
...
function setOrderStep(step) {
  ...
  if (nextDetails) nextDetails.disabled = !selectedHero;
}
...
function beginHeroStep() {
  ...
  if (heroSection) {
    setTimeout(() => heroSection.scrollIntoView({ behavior: "smooth", block: "start" }), 60);
  }
...
function updateStepHeroPreview(h) {
  ...
  box.innerHTML = `
    ...
    <div><span>Current selection</span><strong>${h.name}</strong><small>${h.role}</small></div>
  `;
}
...
function updateFinalReview() {
  ...
  const rate = getRate(target);
  const total = rate > 0 ? formatPeso(rate * stars) : "₱—";

  const rankEl = $("#reviewRank");
  const starsEl = $("#reviewStars");
  const roleEl = $("#reviewRole");
  const heroEl = $("#reviewHero");
  const totalEl = $("#reviewTotal");

  if (rankEl) rankEl.textContent = `${current} → ${target}`;
  if (starsEl) starsEl.textContent = String(stars);
  if (roleEl) roleEl.textContent = role;
  if (heroEl) heroEl.textContent = selectedHero ? selectedHero.name : "No hero selected";
...
function setupOrderFlow() {
  ...
  ["currentRank", "targetRank", "stars", "preferredRole"].forEach(id => {
    const el = document.getElementById(id);
    if (el) {
      el.addEventListener("input", updateFinalReview);
      el.addEventListener("change", updateFinalReview);
    }
  });
...
function createOrderMessage() {
  if (orderStep !== 3) {
    showToast("Complete Rank → Hero → Details first.");
    return;
  }
  ...
  const detailForm = $("#orderStep3");
  const customerName = $("#customerName")?.value.trim() || "";
  const mlId = $("#mlId")?.value.trim() || "";
  const serverId = $("#server")?.value.trim() || "";

  if (detailForm && !detailForm.querySelector("#customerName")?.checkValidity()) {
    $("#customerName")?.focus();
    showToast("Enter your in-game name.");
    return;
  }

  if (detailForm && !detailForm.querySelector("#mlId")?.checkValidity()) {
    $("#mlId")?.focus();
    showToast("Enter your MLBB Account ID.");
    return;
  }

  if (detailForm && !detailForm.querySelector("#server")?.checkValidity()) {
    $("#server")?.focus();
    showToast("Enter your Server ID.");
    return;
  }
  ...
  const lines = [
    ...
    `Preferred Hero: ${selectedHero.name}`,
    `Preferred Role: ${role}`,
