...
/* ===================== CONFIGURATION ===================== */
const SITE = {
  messengerUsername: "https://www.facebook.com/profile.php?id=61592788734201", // Full Facebook profile URL or your actual username
  gcashNumber: "09757209196"                   // Replace with your GCash number
};
...
/* ===================== MESSENGER LINK RESOLVER ===================== */
/*
  Accepts:
  1) a Facebook username, e.g. "ejpilotservice"
  2) a full Facebook profile URL
  3) an m.me URL
  4) a numeric Facebook/Page ID
*/
function buildMessengerUrl(message) {
  const raw = String(SITE.messengerUsername || "").trim();
  if (!raw || raw.includes("YOUR_FACEBOOK_USERNAME")) return "";
  ...
  try {
    if (/^https?:\/\//i.test(raw)) {
      ...
      } else if (host.endsWith("facebook.com") && parsed.pathname.startsWith("/messages/t/")) {
        target = `https://www.facebook.com${parsed.pathname}`;
      } else if (host.endsWith("facebook.com") && parsed.pathname === "/profile.php") {
        const id = parsed.searchParams.get("id");
        if (id) target = `https://www.facebook.com/messages/t/${encodeURIComponent(id)}`;
      } else if (host.endsWith("facebook.com")) {
        const parts = parsed.pathname.split("/").filter(Boolean);
        if (parts.length === 1 && parts[0] !== "profile.php") {
          target = `https://m.me/${encodeURIComponent(parts[0])}`;
        }
      }
    } else if (/^\d+$/.test(raw)) {
      target = `https://www.facebook.com/messages/t/${encodeURIComponent(raw)}`;
    } else {
      const username = raw.replace(/^@/, "").replace(/\/$/, "");
      target = `https://m.me/${encodeURIComponent(username)}`;
    }
  } catch (error) {
    console.error("Messenger URL error:", error);
    return "";
  }
...
function createOrderMessage() {
  ...
  if (!url) {
    showToast("Messenger link is not configured correctly.");
    return;
  }

  // Direct navigation is more reliable than popups inside mobile/in-app browsers.
